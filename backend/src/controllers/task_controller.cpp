...
crow::response createTask(const crow::request& req, AuthMiddleware::context& ctx) {
    try {
        ...
        if (!body) return standard_response(400, false, "Invalid JSON");

        std::string title = body.has("title") ? get_string_safely(body["title"]) : "";
        std::string description = body.has("description") ? get_string_safely(body["description"]) : "";
        std::string due_date = body.has("due_date") ? get_string_safely(body["due_date"]) : "";
...
crow::response updateTask(const crow::request& req, AuthMiddleware::context& ctx, int task_id) {
    try {
        ...
        if (body.has("is_done")) {
            ...
            bool is_done = false;
            if (body["is_done"].t() == crow::json::type::True) is_done = true;
            else if (body["is_done"].t() == crow::json::type::False) is_done = false;
        ...
        if (body.has("due_date")) {
            ...
            params.push_back(d.empty() ? "NULL" : d); // allows clearing due date conceptually, though prepared statements bind as strings. In a full ORM, you'd handle NULL distinctly.
        }
        ...
        // By default, our DB class binds everything as string. A literal "NULL" string won't become a SQL NULL easily via bind unless specifically handled. 
        // For simplicity, we assume empty string means no due date (or let it fail silently and store the string).
        // Since due_date is DATETIME, MySQL will reject "NULL" string. Better logic:
