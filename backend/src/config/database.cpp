...
bool Database::connect(const DbConfig &config) {
  ...
  if (mysql_real_connect(conn, config.db_host.c_str(), config.db_user.c_str(),
                         config.db_pass.c_str(), config.db_name.c_str(), 0,
                         nullptr, 0) == nullptr) {
    Logger::error("mysql_real_connect() failed: " +
...
bool Database::execute(const std::string &query) {
  ...
  if (mysql_query(conn, query.c_str())) {
    std::cerr << "INSERT/UPDATE/DELETE failed: " << mysql_error(conn)
              << std::endl;
    return false;
  }
  return true;
...
bool Database::execute(const std::string &query,
  ...
  if (mysql_stmt_prepare(stmt, query.c_str(), query.length())) {
    Logger::error("mysql_stmt_prepare failed: " +
  ...
  if (params.size() > 0 && mysql_stmt_bind_param(stmt, binds.data())) {
    Logger::error("mysql_stmt_bind_param failed: " +
                  std::string(mysql_stmt_error(stmt)));
    mysql_stmt_close(stmt);
    return false;
  }
  ...
  if (mysql_stmt_execute(stmt)) {
    Logger::error("mysql_stmt_execute failed: " +
                  std::string(mysql_stmt_error(stmt)));
    mysql_stmt_close(stmt);
    return false;
  }
